...
public class PluginManagerMock implements PluginManager {
     ...
     */
    private PluginDescriptionFile findPluginDescription(Class<? extends JavaPlugin> class1) throws IOException,
        ...
        throw new FileNotFoundException(String.format("Could not find file plugin.yml. Maybe forgot to add the 'main'" +
                " property?"));
    }
    ...
    public void registerEvent(Class<? extends Event> event, Listener listener, EventPriority priority,
        ...
        List<ListenerEntry> listeners = eventListeners.get(plugin);
        listeners.add(new ListenerEntry(plugin, listener, executor, event));
    }
